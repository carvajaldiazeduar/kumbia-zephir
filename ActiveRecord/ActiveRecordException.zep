...
class ActiveRecordException extends CoreException
{
	...
	public static function displayWarning(title, message, source){
		String controllerName, action;

		let controllerName = Router::getController();
		let action = Router::getAction();
		
		Kumbia\Flash::warning(
			"<span style='font-size:16px;color:black'>KumbiaWarning: title</span><br/>".
            "<div>".message."<br>".
            "<span style='font-size:12px;color:black'>En el modelo <i>".source."</i> al ejecutar <i>".controllerName."/".action."</i></span></div>", 
            true
		);
